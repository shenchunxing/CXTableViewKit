...
@interface CXTableViewManager ()<UITableViewDelegate, UITableViewDataSource,CXTableViewItemDelegate>
...
@end
@implementation CXTableViewManager
...
#pragma mark - CXTableViewItemDelegate
- (void)filletedCornersForCell:(UITableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath {
    
    CXTableViewSection *section = self.mutableSections[indexPath.section];
    if (!section.needFillet) {
        return;
    }
    Class cellClass = [self classForCellAtIndexPath:indexPath];
    CGFloat cellHeight = 0;
    if ([cellClass conformsToProtocol:@protocol(CXTableViewCellProtocol)]) {
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    if (self.mutableSections.count > section){
        CXTableViewSection *tableSection = self.mutableSections[section];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.mutableSections.count > indexPath.section){
        ...
        if (section.needFillet) {
            CXTableViewItem *item = [self itemAtIndexPath:indexPath];
...
- (CXTableViewItem *)itemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.mutableSections.count > indexPath.section) {
        CXTableViewSection *section = self.mutableSections[indexPath.section];
        section.isLastSectionInManager = indexPath.section == self.mutableSections.count - 1;
        if ([section.items count] > indexPath.row) {
            CXTableViewItem *item = section.items[indexPath.row];
...
#pragma mark - getter
- (NSArray<CXTableViewSection *> *)sections {
